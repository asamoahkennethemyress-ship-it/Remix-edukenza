...
export const Navbar: React.FC = () => {
  ...
  return (
    <motion.header 
      ...
      className="sticky top-0 z-50 bg-[#002147] text-white border-b-4 border-[#D4AF37] shadow-xl pt-[env(safe-area-inset-top,0px)]"
    >
        ...
        <div className="flex items-center justify-between h-16">
            ...
            {navItems.map((link) => {
              ...
              return (
                <motion.button
                  ...
                  className={`relative py-1.5 transition-colors cursor-pointer border-b-2 ${
                    isActive
          ...
          <div className="hidden lg:flex items-center gap-3">
            <motion.button
              ...
              whileTap={shouldReduceMotion ? {} : { scale: 0.98 }}
              className="btn-interactive px-3 py-1.5 text-xs font-bold text-amber-300 hover:text-white bg-[#001733] hover:bg-white/10 transition-colors border border-[#D4AF37]/40 rounded-lg flex items-center gap-1.5 cursor-pointer shadow-xs"
            ...
            {currentUser ? (
              <div className="flex items-center gap-3 bg-white/10 px-3 py-1.5 rounded-lg border border-white/20">
                ...
                <motion.button
                  ...
                  className="btn-interactive ml-2 text-xs bg-[#D4AF37] text-[#002147] font-bold px-2.5 py-1 rounded transition hover:bg-[#c29f2e] cursor-pointer"
                >
                  Dashboard
                </motion.button>
                <button
                  onClick={logout}
                  title="Log out"
                  className="btn-interactive text-slate-300 hover:text-red-400 p-1 transition cursor-pointer"
                >
              ...
              <motion.button
                ...
                whileTap={shouldReduceMotion ? {} : { scale: 0.98 }}
                className="btn-interactive px-5 py-2 text-xs uppercase font-bold text-white hover:text-[#D4AF37] bg-white/10 hover:bg-white/20 transition-colors border border-white/20 rounded shadow-sm cursor-pointer"
              >
                Login
              </motion.button>
            )}
          ...
          <div className="flex lg:hidden items-center gap-2">
            <button
              ...
              className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-slate-200 hover:text-white rounded-lg bg-[#001733] cursor-pointer transition-colors"
      ...
      {/* Mobile Drawer with smooth slide open/close */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            className="lg:hidden bg-[#001733] border-t border-[#D4AF37]/30 px-4 pt-3 pb-[calc(1.5rem+env(safe-area-inset-bottom,0px))] space-y-3 overflow-hidden"
          >
            <button
              onClick={() => handleNavClick('home')}
              className={`w-full min-h-[44px] flex items-center text-left px-4 py-2.5 rounded text-xs font-bold uppercase tracking-wider cursor-pointer transition-colors ${
                activeView === 'home' ? 'bg-[#D4AF37] text-[#002147]' : 'text-slate-200 hover:bg-white/5'
              }`}
            >
              Home
            </button>
            {navItems.map((link) => (
              <button
                ...
                onClick={() => handleNavClick(link.view)}
                className={`w-full min-h-[44px] flex items-center text-left px-4 py-2.5 rounded text-xs font-bold uppercase tracking-wider cursor-pointer transition-colors ${
                  activeView === link.view ? 'bg-[#D4AF37] text-[#002147]' : 'text-slate-200 hover:bg-white/5'
                }`}
              >
                {link.label}
              </button>
            ...
            <div className="pt-4 border-t border-white/10 flex flex-col gap-2">
              <button
                ...
                className="w-full py-2.5 px-4 text-left text-xs font-bold uppercase text-amber-300 border border-[#D4AF37]/40 rounded hover:bg-white/10 transition-colors flex items-center gap-2"
              >
              ...
              {currentUser ? (
                ...
                <button
                  onClick={() => handleNavClick('login')}
                  className="w-full py-2.5 text-center text-xs font-bold uppercase text-slate-200 border border-white/20 rounded hover:border-[#D4AF37] hover:text-[#D4AF37] transition-colors"
                >
                  Login
                </button>
              )}
